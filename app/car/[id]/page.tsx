...
export default async function CarPage({ params }: { params: { id: string } }) {
  ...
  const specs: [string, any][] = [
    ["Year", car.year], ["Mileage", `${Number(car.mileage).toLocaleString()} km`],
    ["Origin", `${FLAG[car.origin]} ${car.origin}`], ["Gearbox", car.gearbox],
    ["Fuel", car.fuel], ["Engine", car.engine || "—"], ["Power", car.power || "—"],
    ["Colour", car.color || "—"], ["CO₂", car.co2 ? `${car.co2} g/km` : "—"],
    ["NOx", car.nox ? `${car.nox} mg/km` : "—"],
  ];
  return (
    <main>
      <Link href="/" className="backLink">← Back to stock</Link>
      <div className="detailWrap">
        <CarGallery imgs={imgs} />
        <div>
          ...
          {isSold
            ? <div className="ok">This car has been sold. Browse our stock or ask us to source one like it.</div>
